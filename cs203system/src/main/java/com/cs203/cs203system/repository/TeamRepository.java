...
public interface TeamRepository extends JpaRepository<Team, Integer> {
    ...
    List<Team> findByTournamentOrderByPointsDesc(Tournament tournament);

    // Finds teams by tournament and their status (e.g., QUALIFIED, ELIMINATED)
    List<Team> findByTournamentAndStatus(Tournament tournament, Team.Status status);

    // Finds teams by tournament and bracket type for more specific queries
    @Query("SELECT DISTINCT t FROM Team t JOIN t.matches m WHERE m.tournament = :tournament AND m.bracket = :bracket")
    List<Team> findByTournamentAndBracket(@Param("tournament") Tournament tournament, @Param("bracket") Match.Bracket bracket);
}